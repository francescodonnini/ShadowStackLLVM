...
typedef pte_t *(*__pte_offset_map_lock_t)(struct mm_struct *mm, pmd_t *pmd, unsigned long address, spinlock_t **ptlp);
...
static inline pte_t *my_pte_alloc_map_lock(struct mm_struct *mm, pmd_t *pmd, unsigned long address, spinlock_t **ptlp) {
    ...
    return my__pte_offset_map_lock(mm, pmd, address, ptlp);
}
    #define compat_pte_offset_map_lock(mm, pmd, addr, ptlp) \
        my__pte_offset_map_lock(mm, pmd, addr, ptlp)
#else
    #define compat_pte_offset_map_lock(mm, pmd, addr, ptlp) \
